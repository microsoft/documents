...
Uses the Azure OpenAI-compatible ``/images/generations`` endpoint.
Authentication is handled through :class:`~azure.identity.DefaultAzureCredential`.
...
def resolve_image_prompt(
    ...
    """If the section has an ``image_prompt``, generate the image and set ``image``."""
    ip = section_data.get("image_prompt")
    if not ip:
        return
    existing = section_data.get("image")
    ...
    try:
        ...
        print(f"Warning: image generation failed for section — {exc}")
        return
    if path:
