"""CLI entry point for the document generator."""
...
def _load_env() -> None:
    ...
    Searches (in order):
    1. ``<cwd>/.env``
    2. ``<cwd>/.azure/documents/.env``  (azd environment)
    """
    try:
...
def main(argv: list[str] | None = None) -> None:
    """Parse CLI arguments and generate the document."""
    _load_env()
    parser = argparse.ArgumentParser(
        description="Generate a Word document from a spec file.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Section numbers to generate (1-indexed). "
            "Examples: '5' (one section), '3-7' (range), '1,3,5-8' (mixed). "
    ...
    render(
        ...
        spec_path=args.spec,
        section_selection=args.sections,
    )
